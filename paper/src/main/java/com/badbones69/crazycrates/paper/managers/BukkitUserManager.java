...
public class BukkitUserManager extends UserManager {
    ...
    /**
     * 获取玩家在指定箱子的开箱次数（用于保底系统）
     * @param uuid 玩家UUID
     * @param crateName 箱子名称
     * @return 开箱次数
     */
    public int getOpenCount(@NotNull final UUID uuid, @NotNull final String crateName) {
        return this.data.getConfiguration().getInt("Players." + uuid + ".OpenCount." + crateName, 0);
    }
     ...
     * @param crateName 箱子名称
     */
    public void addOpenCount(@NotNull final UUID uuid, @NotNull final String crateName) {
        final int current = getOpenCount(uuid, crateName);
        final YamlConfiguration configuration = this.data.getConfiguration();
        configuration.set("Players." + uuid + ".OpenCount." + crateName, current + 1);
        this.data.save();
    }

    /**
     * 重置玩家在指定箱子的开箱次数（触发保底后）
     * @param uuid 玩家UUID
     * @param crateName 箱子名称
     */
    public void resetOpenCount(@NotNull final UUID uuid, @NotNull final String crateName) {
        final YamlConfiguration configuration = this.data.getConfiguration();
        configuration.set("Players." + uuid + ".OpenCount." + crateName, 0);
        this.data.save();
    }
}