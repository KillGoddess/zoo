...
public class PlaceholderAPISupport extends PlaceholderExpansion {
    ...
    public @NotNull final String onPlaceholderRequest(final Player player, @NotNull final String identifier) {
        ...
        // 保底次数变量
        if (identifier.contains("_pity_count")) {
            final String crateName = identifier.replace("_pity_count", "");
            values.add(String.valueOf(this.userManager.getOpenCount(uuid.get(), crateName)));
        }
        ...
        // 保底次数变量（支持其他玩家）
        if (parsed.equalsIgnoreCase("%s_%s_pity_count".formatted(playerName, crateName))) {
            values.add(String.valueOf(this.userManager.getOpenCount(id, crateName)));
        }
