...
public class CrateInteractListener implements Listener {
        ...
        switch (action) {
            ...
            case RIGHT_CLICK_BLOCK, RIGHT_CLICK_AIR -> {
                ...
                if (isRightClickToOpen) { // right click to open
                    // Check if player is sneaking (Shift + Right Click) for 5-pull
                    if (player.isSneaking()) {
                        openCrateFiveTimes(player, crateLocation, crate);
                    } else {
                        openCrate(player, crateLocation, crate);
                    }
    ...
    /**
     * Opens a crate 5 times for a player (5-pull feature).
     * Requires player to have at least 5 keys (physical or virtual).
     */
    private void openCrateFiveTimes(@NotNull final Player player, @NotNull final CrateLocation crateLocation, @NotNull final Crate crate) {
        final KeyCheckEvent key = new KeyCheckEvent(player, crateLocation);
        ...
        final String fileName = crate.getFileName();
        final String fancyName = crate.getCrateName();
        final int requiredKeys = 5; // 5-pull requires 5 keys

        // Check if player has at least 5 keys
        final int totalKeys = this.userManager.getTotalKeys(player.getUniqueId(), fileName);

        if (totalKeys < requiredKeys) {
            // Send message: not enough keys for 5-pull
            player.sendRichMessage("<red>你需要至少 5 把钥匙才能进行五连抽！当前钥匙数量: " + totalKeys);
        ...
        // Check if inventory has space (need space for 5 prizes)
        if (MiscUtils.isInventoryFull(player)) {
            Messages.inventory_not_empty.sendMessage(player, "{crate}", fancyName);
            return;
        }

        // Determine key type priority: physical > virtual
        final ItemStack itemStack = player.getInventory().getItemInMainHand();
        final int physicalKeys = this.userManager.getPhysicalKeys(player.getUniqueId(), fileName);
        final int virtualKeys = this.userManager.getVirtualKeys(player.getUniqueId(), fileName);

        KeyType keyType;
        boolean hasEnoughPhysical = false;

        if (this.config.getProperty(ConfigKeys.physical_accepts_physical_keys) && 
            crate.getCrateType() != CrateType.crate_on_the_go && 
            ItemUtils.isSimilar(itemStack, crate) && 
            physicalKeys >= requiredKeys) {
            keyType = KeyType.physical_key;
            hasEnoughPhysical = true;
        } else if (this.config.getProperty(ConfigKeys.physical_accepts_virtual_keys) && virtualKeys >= requiredKeys) {
            keyType = KeyType.virtual_key;
        } else {
            // Mixed keys or not enough of one type
            player.sendRichMessage("<red>五连抽需要 5 把相同类型的钥匙！实体钥匙: " + physicalKeys + ", 虚拟钥匙: " + virtualKeys);
            lackingKey(player, crate, location, false);
            key.setCancelled(true);
            return;
        }
        ...
        // Take 5 keys at once
        boolean keysTaken = this.userManager.takeKeys(player.getUniqueId(), fileName, keyType, requiredKeys, hasEnoughPhysical);

        if (!keysTaken) {
            player.sendRichMessage("<red>扣除钥匙失败！请联系管理员。");
            key.setCancelled(true);
            return;
        }

        // Notify player
        player.sendRichMessage("<green>开始五连抽！正在抽取 5 个奖品...");

        // Give 5 prizes directly without animation
        for (int i = 1; i <= 5; i++) {
            final com.badbones69.crazycrates.paper.api.objects.Prize prize = crate.pickPrize(player);
            
            if (prize != null) {
                prize.givePrize(player);
                player.sendRichMessage("<yellow>第 " + i + " 抽: <green>" + prize.getPrizeName());
            }
        }

        player.sendRichMessage("<green>五连抽完成！");
    }
}